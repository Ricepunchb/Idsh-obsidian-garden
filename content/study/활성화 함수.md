...
### 4.5. GLU / SwiGLU (Gated Linear Unit)
"게이팅(Gating)" 아이디어를 활성화 함수에 접목한 방식이다. 입력을 두 개의 선형 사영(Linear Projection)으로 나눠서, 하나는 **내용(Content)**, 다른 하나는 그 내용을 얼마나 통과시킬지 정하는 **게이트(Gate)**로 쓴다.
$$
\text{GLU}(x) = (xW_1) \odot \sigma(xW_2)
$$
여기서 $\odot$는 원소별 곱(element-wise product)이다. 게이트 값이 활성화 함수(원래는 sigmoid)를 거치므로 각 차원을 얼마나 열지/닫을지 입력에 따라 동적으로 결정된다.

**SwiGLU**는 게이트 쪽 활성화 함수를 Swish로 바꾼 버전으로, 현재 Llama·PaLM 등 대부분의 최신 LLM이 FFN에 채택하고 있다 ([[Transformer]] 참고).
$$
\text{SwiGLU}(x) = (xW_1) \odot \text{Swish}(xW_2)
$$
- **특징**: 일반 FFN보다 파라미터(행렬)가 하나 더 필요하지만(3개의 $W$), 표현력과 학습 안정성이 좋아 성능 이득이 크다.

> [!tip] 왜 비선형 함수가 반드시 필요한가
> 비선형 활성화 함수가 없으면 아무리 층을 쌓아도 $W_1 W_2 x = Wx$처럼 하나의 선형 변환으로 합쳐진다. 즉 층을 늘려도 표현력이 늘지 않는다. 비선형성이 있어야 층을 쌓을수록 더 복잡한 함수를 근사할 수 있다.
...
-   **온도(Temperature) 적용**: 샘플링 시 분포의 뾰족함(sharpness)을 조절하기 위해 온도 $T$로 나눈 뒤 softmax를 적용한다.
    $$
    \text{softmax}(z/T)_i = \frac{e^{z_i/T}}{\sum_j e^{z_j/T}}
    $$
    $T\to 0$이면 argmax에 가까워지고(분포가 뾰족해짐), $T\to\infty$이면 균등분포에 가까워진다(분포가 평평해짐).
...
| **SwiGLU**       | $(xW_1)\odot\text{Swish}(xW_2)$ | 입력에 따라 동적으로 게이팅   | 파라미터(행렬) 하나 더 필요      | **최신 LLM의 FFN (Llama, PaLM)** |
